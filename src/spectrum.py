...
if __name__ == '__main__':
    ...
    spec = spec.range(900, 1700, x=True)
    der0 = spec * 1
    spec.get_derivative(2)
    print(len(spec.get_extrema()[0]))
    # show_spectra([spec])
    f, axes = plt.subplots(2, 1, sharex=True)

    # axes[1].plot(spec.wavenums, spec.data)
    # axes[1].set_ylabel('2nd der.', fontdict={'fontsize': 8})
    # axes[0].plot(der0.wavenums, der0.data)
    # axes[0].set_ylabel('spectrum', fontdict={'fontsize': 8})
    # axes[1].set_xlabel('wavenumbers', fontdict={'fontsize': 8})
    # plt.show()
